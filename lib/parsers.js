...
module.exports.dIStsPollIngCameraEvent = (api, states, res) => {
    event.emit('debug', 'dIStsPollIngCameraEvent - Response: ' + JSON.stringify(res));
    /*if (res && res.PDParam && res.PDParam.triggerMotion){
        ...
        try {
            ...
            if (res && res.radios){
                ...
                Object.keys(states.AudioStation.players).forEach((playerid) => {
                    states.AudioStation.players[playerid].FavoriteRadioPlaylist = JSON.stringify(radio_playlist);
                });
            }
        } catch (e) {
